...
Reference: https://github.com/iterative/dvc/blob/0.94.0/dvc/dagascii.py
"""
...
def string_pager(text):
    global __string_buffer
    __string_buffer += text


def get_string_and_clear():
    global __string_buffer
    ret_str = __string_buffer
    __string_buffer = ""
    return ret_str
...
def _build_sugiyama_layout(vertexes, edges):
    ...
    sug.yspace = 5  # VertexViewer.HEIGHT
    sug.xspace = minw
...
def draw_to_string(edges, vertexes=None):
    """Build a DAG, draw it and output to string.

    Args:
        edges (list): list of graph edges.
        vertexes (list, optional): list of graph vertexes. If the value is `None`, it will be generated from edges.
    """
    if vertexes is None:
        vertexes = list(set(sum([list(e) for e in edges], [])))

    # hack to reverse the graph so that it displays from top-to-bottom (DVC original implementation is bottom-to-top)
    edges = [(e[1], e[0]) for e in edges]

    draw(vertexes, edges)
    return get_string_and_clear()


if __name__ == "__main__":
    E = [(0, 1), (0, 2), (1, 3), (2, 3), (4, 0), (1, 4), (4, 5), (5, 6), (3, 6), (3, 7), (6, 8), (7, 8), (8, 9), (5, 9)]

    print(draw_to_string(E))
